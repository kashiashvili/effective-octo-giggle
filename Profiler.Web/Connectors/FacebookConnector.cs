...
public class FacebookConnector : IConnector
{
    ...
    public async Task<ProfileData> FetchAsync(CancellationToken cancellationToken = default)
    {
        ...
        using var likes = await TryGetJson($"{baseUrl}/me/likes?fields=category&access_token={_token}", cancellationToken);
        ...
        using var interests = await TryGetJson($"{baseUrl}/me/interests?fields=name&access_token={_token}", cancellationToken);
        ...
        using var me = await TryGetJson($"{baseUrl}/me?fields=education&access_token={_token}", cancellationToken);
