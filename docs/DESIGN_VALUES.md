...
- **Worldview.** *Primal world beliefs* (Clifton et al. 2019; PI-18/PI-6) are validated, stable beliefs about the world's basic character — **Safe** (vs dangerous), **Enticing** (vs dull), **Alive** (vs mechanistic) under one **Good** belief. Safe and Enticing are neither political nor moral and are the everyday meaning of "how someone sees the world"; Alive edges into spirituality and is left out.
...
Interests page → "Next: what matters to you (2 minutes, optional)" on save; matches page nudge when missing (like the intent nudge); dashboard quick action second when missing, with the profile summary once set; profile page link; values page explains the science in three sentences with sources; landing step 1 mentions it.
...
Unit: derivation rules, centring removes scale use, hedonism split, reversal, JSON round-trip, tiers, explanations. Synthetic resolution test (population with realistic priority structure + noise): each dimension uses all five levels, random pairs are **not** mostly "similar", pairs from the same latent profile read similar/overlap far more often than independent pairs. Integration: answers absent from every table, consent required, removal, card line and explanation, sort, withheld while hidden, export. Then Release Auditor and a QA walk.
